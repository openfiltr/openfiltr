...
- PR #132 (merged 2026-03-30): added ADR-0003 to record bbolt as the default embedded database backend with PostgreSQL as an optional secondary, and marked ADR-0001 as superseded; aligned `AGENTS.md`, `.github/copilot-instructions/project.md`, `README.md`, `ROADMAP.md`, and the bbolt migration tracker with the bbolt-first runtime reality instead of the previously documented PostgreSQL-only position.
...
- Removed the post-merge changelog issue workflow `.github/workflows/changelog-update.yml`; changelog updates now stay in the implementation PR instead of opening follow-up GitHub issues after merge.
